...
public class Draft_10 extends WebSocketProtocol {
    ...
    public static int readVersion(Handshake handshakedata) {
    ...
    private Message fragmentedframe = null;
    ...
    public ByteBuffer createBinaryFrame(Message framedata) {
        ...
        ByteBuffer buf = ByteBuffer.allocate(1 + (sizebytes > 1 ? sizebytes + 1 : sizebytes) + (mask ? 4 : 0)
                + mes.remaining());
    ...
    public List<Message> createFrames(ByteBuffer binary, boolean mask) {
        Message curframe = new MessageImpl();
        try {
            curframe.setPayload(binary);
        } catch (WsIOException e) {
            throw new RuntimeException("Not sendable" + e);
        }
        curframe.setFin(true);
        curframe.setOptcode(Opcode.BINARY);
        curframe.setTransferemasked(mask);
        return Collections.singletonList((Message) curframe);
    }
    ...
    public List<Message> createFrames(String text, boolean mask) {
        Message curframe = new MessageImpl();
        try {
            curframe.setPayload(ByteBuffer.wrap(CharsetStringConverter.utf8Bytes(text)));
        } catch (WsIOException e) {
            throw new RuntimeException("Not sendable" + e);
        }
        curframe.setFin(true);
    ...
    @Override
    public List<Message> translateFrame(ByteBuffer buffer) throws LimitExedeedException, WsIOException {
        List<Message> frames = new LinkedList<Message>();
        Message cur;
    ...
    public Message translateSingleFrame(ByteBuffer buffer) throws IncompleteException, WsIOException {
