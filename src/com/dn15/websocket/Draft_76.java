...
public class Draft_76 extends Draft_75 {
    ...
    public HandshakeState acceptHandshake(ClientHandshake request, ServerHandshake response) {
        ...
        try {
            ...
            if (Arrays.equals(
                    content,
                    createChallenge(request.getFieldValue("Sec-WebSocket-Key1"),
    ...
    public Handshake translateHandshake(ByteBuffer buf) throws InvalidHandshakeException {
        ...
        Handshake bui = translateHandshakeHttp(buf, role);
    ...
    public List<Message> translateFrame(ByteBuffer buffer) throws WsIOException {
        buffer.mark();
        List<Message> frames = super.translateRegularFrame(buffer);
        if (frames == null) {
            buffer.reset();
            frames = readyframes;
            readingState = true;
            ...
            if (!currentFrame.hasRemaining()) {
                if (Arrays.equals(currentFrame.array(), closehandshake)) {
                    frames.add(new CloseMessageBuilder(CloseCodes.NORMAL_CLOSURE));
                    return frames;
                } else {
                    throw new InvalidFrameException();
                }
